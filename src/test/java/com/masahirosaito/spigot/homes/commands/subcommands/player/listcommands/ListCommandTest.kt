...
import com.masahirosaito.spigot.homes.strings.EconomyStrings.createPayMessage
...
class ListCommandTest {
    ...
    fun コマンドを実行した場合にホームリストを表示する() {
        ...
        assertEquals(nepian.lastMsg(), createHomeListMessage(PlayerDataManager.findPlayerData(nepian), false))
    }
    ...
    fun 料金が設定されたコマンドの実行時に所持金が足りなかった場合はエラーを表示する() {
        ...
        nepian.executeHomeCommand("list")
        assertEquals(nepian.lastMsg(), createNotEnoughMoneyErrorMessage(homes.fee.LIST))
    }
    ...
    fun 料金が設定されたコマンドの実行時に所持金が足りている場合は所持金を消費しコマンドを実行する() {
        ...
        nepian.executeHomeCommand("list")
        assertEquals(nepian.lastMsg(), createPayMessage(1000.0.toString(), 4000.0.toString()))
    }
}
